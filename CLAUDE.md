...
- `strip_instances()` in stripper.py: wraps with `ifdef FPGA_SYN`/`else`/`endif`; **auto un-strips all then re-strips**; comment-safe instance matching (skips `//`/`/* */`); port connection parser skips comments to avoid `)` false matches; non-ANSI multi-line port continuation handled; idempotent on re-run; unstrip loop limit 1000
